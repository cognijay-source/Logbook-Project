  ...
  return (
    <div className="space-y-3">
      <p className="text-sm font-medium">
        {flights.length} flight{flights.length !== 1 ? 's' : ''} extracted —
        review and edit before importing
      </p>
      <p className="text-muted-foreground text-xs">
        Fields highlighted in yellow could not be read clearly and need review.
        Empty fields were not found on the page.
      </p>
          ...
          <TableHeader>
            <TableRow>
              ...
              {columns.map((col) => (
                <TableHead
                  key={col.key}
                  className="min-w-[80px] text-xs whitespace-nowrap"
                >
                  {col.label}
                </TableHead>
              ))}
                  ...
                  return (
                    <TableCell key={col.key} className="p-1">
                      <Input
                        ...
                        onChange={(e) =>
                          updateField(rowIdx, col.key, e.target.value)
                        }
                      />
                    </TableCell>
                  )
